...
SwiftLanguageRuntime::GetRuntimeUnwindPlan(ProcessSP process_sp,
  ...
  if (in_prologue) {
    ...
    // I think this should also consider the indirect case and have an extra
    // indirection here.
    assert(!indirect_context);
  ...
  if (indirect_context) {
    if (in_prologue) {
      // I think this is wrong. If indirect context, frame X's async register
      // is the async context of frame X-1. The async register of frame X-1
      // (assuming it is also indirect) should contain the async *context* of
      // frame X, as such we should dereference this register once.
      ...
      assert(expr[expr_size - 1] == llvm::dwarf::DW_OP_deref &&
             "Should skip a deref");
      // I think we should dereference this twice instead.
